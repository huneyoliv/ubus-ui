    ...
    Column(modifier = Modifier.fillMaxSize().background(MaterialTheme.colorScheme.background).padding(horizontal = 20.dp)) {
        ...
        BentoCard {
            Column(horizontalAlignment = Alignment.CenterHorizontally, modifier = Modifier.fillMaxWidth()) {
                Icon(Icons.Default.CalendarMonth, null, tint = UbusText3, modifier = Modifier.size(48.dp))
                Spacer(Modifier.height(12.dp))
                Text("Renovação semestral", style = MaterialTheme.typography.titleMedium, color = UbusText3, textAlign = TextAlign.Center)
                Text("Envie seus documentos para renovar o cadastro.", style = MaterialTheme.typography.bodySmall, color = UbusText3, textAlign = TextAlign.Center)

    ...
    Column(modifier = Modifier.fillMaxSize().background(UbusBackground).padding(horizontal = 20.dp)) {
        ...
        BentoCard {
            Column(horizontalAlignment = Alignment.CenterHorizontally, modifier = Modifier.fillMaxWidth()) {
                Icon(Icons.Default.Accessibility, null, tint = UbusMutedForeground, modifier = Modifier.size(48.dp))
                Spacer(Modifier.height(12.dp))
                Text("Baixa mobilidade", style = MaterialTheme.typography.titleMedium, color = UbusMutedForeground, textAlign = TextAlign.Center)
                Text("Solicite prioridade de embarque se você precisa de acessibilidade.", style = MaterialTheme.typography.bodySmall, color = UbusMutedForeground, textAlign = TextAlign.Center)
            }
        }
    }
}
